...
vector<vector<float>> solve_matrix(vector<vector<float>> matrix, vector<float> &target, vector<int> &buttons){
    for (int i = 0; i < target.size(); i++){
...
void apply_solution(Machine &m){
    ...
    for (int i = 0; i < m.rows; i++){
        push_button(m, m.matrix[m.button_order[i]], m.target[i]);
    }
}
...
void analyse(string file){
    ...
    vector<vector<float>> solved_matrix = solve_matrix(machines[1].matrix, machines[1].target, machines[1].button_order);
